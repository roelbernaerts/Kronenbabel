...
/**
 * Converts raw Float32 PCM data to the format expected by Gemini (Int16, base64).
 */
export function createPcmBlob(data: Float32Array): Blob {
  const l = data.length;
  const int16 = new Int16Array(l);
  for (let i = 0; i < l; i++) {
    // Clamp functionality to avoid overflow/distortion
...
export function downsampleBuffer(buffer: Float32Array, inputSampleRate: number, targetSampleRate: number): Float32Array {
  ...
  while (offsetResult < result.length) {
    ...
    // Simple linear interpolation or averaging could go here, but for speech API 
    // simple decimation is often "good enough" or we just pick the nearest neighbor 
    // to keep it fast for real-time.
    // However, averaging reduces aliasing.
    let accum = 0;
    let count = 0;
